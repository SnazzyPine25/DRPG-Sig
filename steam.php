...
$str_sig_file='./signatures/'.$str_input.'.png';
...
if(empty($str_input) || !is_numeric($str_input) || strlen($str_input)!=17) {
	...
	if(file_exists($str_sig_file)) {
		$bSignatureExists=TRUE;
		$FileTime=filemtime($str_sig_file);
		$RealTime=time();
		$Diff=($RealTime-$FileTime);
	}

	if($bSignatureExists && $Diff<120) {
		$res_image_main=imagecreatefrompng($str_sig_file);
	} else {
